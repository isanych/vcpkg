...
namespace vcpkg::Build
{
    ...
    static void get_generic_cmake_build_args(const VcpkgPaths& paths,
                                             ...
                                             std::unordered_map<std::string, std::string>& out_vars)
    {
    ...
    static CompilerInfo load_compiler_info(const VcpkgPaths& paths, const AbiInfo& abi_info)
    {
        ...
        for (const auto& kv : cmake_args)
        {
            cvars.emplace_back(kv.first, kv.second);
        }
        auto command = vcpkg::make_cmake_cmd(paths, paths.ports_cmake, std::move(cvars));
                                                                   ...
                                                                   Triplet triplet)
    {
        ...
        variables.emplace("_VCPKG_EDITABLE", Util::Enum::to_bool(action.build_options.editable) ? "1" : "0"),
        ...
        {
            variables.emplace("_VCPKG_PROHIBIT_BACKCOMPAT_FEATURES", "1");
        }
        ...
        for (const auto& dep : filter_dependencies(scf.core_paragraph->dependencies, triplet, variables))
        {
