...
if(FEATURES MATCHES "tools")
    ...
    foreach(tool IN LISTS TOOLS)
        foreach(suffix IN LISTS TOOL_SUFFIXES)
            ...
            if(EXISTS "${full_tool}")
                file(REMOVE "${full_tool}")
            endif()
            set(full_tool "${CURRENT_PACKAGES_DIR}/bin/${tool}${suffix}")
            if(EXISTS "${full_tool}")
                # get rid of -shared / -static suffix
                set(renamed_tool "${CURRENT_PACKAGES_DIR}/bin/${tool}${VCPKG_TARGET_EXECUTABLE_SUFFIX}")
                if(NOT full_tool STREQUAL "${renamed_tool}")
                    file(RENAME "${full_tool}" "${renamed_tool}")
                    set(full_tool "${renamed_tool}")
                endif()
                file(INSTALL "${full_tool}" DESTINATION "${CURRENT_PACKAGES_DIR}/tools/${PORT}")
                file(REMOVE "${full_tool}")
            endif()
        endforeach()
    endforeach()
