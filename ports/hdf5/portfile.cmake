...
if(FEATURES MATCHES "tools")
    ...
    foreach(tool IN ITEMS h5cc h5hlcc h5c++ h5hlc++ h5copy h5diff h5dump h5ls h5stat gif2h5 h52gif h5clear h5debug h5format_convert h5jam h5unjam h5ls h5mkgrp h5repack h5repart h5watch ph5diff h5import)
        foreach(suffix IN ITEMS "-shared" "-static" "")
            set(binary "${tool}${suffix}${VCPKG_TARGET_EXECUTABLE_SUFFIX}")
            set(full_tool "${CURRENT_PACKAGES_DIR}/bin/${binary}")
            if(EXISTS "${full_tool}")
                vcpkg_clean_tool_in_bin("${tool}${suffix}")
                file(RENAME "${full_tool}" "${CURRENT_PACKAGES_DIR}/tools/${PORT}/${binary}")
            endif()
        endforeach()
    endforeach()
